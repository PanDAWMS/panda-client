...
from pandaclient import Client, MiscUtils, PandaToolsPkgInfo, PLogger, PsubUtils
...
def main(get_taskparams=False, ext_args=None, dry_mode=False):
    ...
    group_config.add_argument(
        ...
        dest="loadJson",
        default=None,
        help="Read task parameters from a json file. Some parameters can be overridden " "by using command-line arguments",
    )
    group_config.add_argument(
        "--dumpJson",
        action="store",
        dest="dumpJson",
        default=None,
        help="Dump all command-line parameters and submission result " "such as returnCode, returnOut, and jediTaskID to a json file",
    )
    group_config.add_argument(
        "--nParallelEvaluation",
        action="store",
    ...
    group_config.add_argument(
        ...
        default=10,
        type=int,
        help="The max number of hyperparameter points to be evaluated in the entire search " "(for each segment in segmented HPO). " "10 by default",
    )
    group_config.add_argument(
        "--maxEvaluationJobs",
        action="store",
    ...
    group_config.add_argument(
        "--steeringContainer", action="store", dest="steeringContainer", default=None, help="The container image for steering run by docker"
    )
    group_config.add_argument(
        "--steeringExec",
        action="store",
    ...
    group_config.add_argument("--evaluationContainer", action="store", dest="evaluationContainer", default=None, help="The container image for evaluation")
    group_config.add_argument("--evaluationExec", action="store", dest="evaluationExec", default=None, help="Execution string to run evaluation in singularity")
    group_config.add_argument(
        "--evaluationInput",
        action="store",
        dest="evaluationInput",
        default="input.json",
        help="Input filename for evaluation where a json-formatted hyperparameter point is placed. " "input.json by default",
    )
    group_config.add_argument(
    ...
    group_config.add_argument(
        "--evaluationOutput", action="store", dest="evaluationOutput", default="output.json", help="Output filename of evaluation. output.json by default"
    )
    group_config.add_argument("--evaluationMeta", action="store", dest="evaluationMeta", default=None, help="The name of metadata file produced by evaluation")
    group_config.add_argument(
        "--evaluationMetrics", action="store", dest="evaluationMetrics", default=None, help="The name of metrics file produced by evaluation"
    )
    group_config.add_argument(
        "--checkPointToSave",
        action="store",
        dest="checkPointToSave",
        default=None,
    ...
    group_config.add_argument(
        ...
        default=None,
        type=int,
        help="Frequency to check files for checkpointing in minute. " "5 by default",
    )
    group_config.add_argument(
        "--alrbArgs",
        action="store",
    ...
    group_input.add_argument("--trainingDS", action="store", dest="trainingDS", default=None, help="Name of training dataset")
    ...
    group_submit.add_argument("--workingGroup", action="store", dest="workingGroup", default=None, help="set working group")
    ...
    taskParamMap["multiStepExec"] = {
        ...
        "containerOptions": {
            "containerExec": "while [ ! -f __payload_in_sync_file__ ]; do sleep 5; done; "
    ...
    if options.checkPointToSave is not None:
        taskParamMap["multiStepExec"]["coprocess"] = {"command": "${TRF}", "args": "--coprocess ${TRF_ARGS}"}
    ...
    taskParamMap["jobParameters"] = [
        {"type": "constant", "value": '-o {} -j "" --inSampleFile {}'.format(options.evaluationOutput, options.evaluationInput)},
        {"type": "constant", "value": f"-a {archiveName} --sourceURL {sourceURL}"},
    ]
    ...
    taskParamMap["jobParameters"] += PsubUtils.convertParamStrToJediParam(options.evaluationExec, {}, "", True, False, includeIO=False)
    ...
    if options.checkPointToSave is not None:
        taskParamMap["jobParameters"] += [
            {"type": "constant", "value": f"--checkPointToSave {options.checkPointToSave}"},
        ]
        if options.checkPointInterval is not None:
            taskParamMap["jobParameters"] += [
                {"type": "constant", "value": f"--checkPointInterval {options.checkPointInterval}"},
            ]

    if options.checkPointToLoad is not None:
        taskParamMap["jobParameters"] += [
            {"type": "constant", "value": f"--checkPointToLoad {options.checkPointToLoad}"},
        ]

    if options.trainingDS is not None:
        taskParamMap["jobParameters"] += [
            {"type": "constant", "value": f"--writeInputToTxt IN_DATA:{options.evaluationTrainingData}"},
            {
                "type": "template",
                "param_type": "input",
                "value": '-i "${IN_DATA/T}"',
    ...
    if options.segmentSpecFile is not None:
        ...
        with open(options.segmentSpecFile) as f:
            ...
            # search space
            if "opt_space" in taskParamMap["hpoRequestData"] and isinstance(taskParamMap["hpoRequestData"]["opt_space"], dict):
