...
def make_arg_parse():
    ...
    optP.add_argument("--version", action="store_const", const=True, dest="version", default=None, help="Displays version")
    optP.add_argument(
        "--loadJson",
        action="store",
        dest="loadJson",
    ...
    optP.add_argument(
        ...
        dest="dumpJson",
        default=None,
        help="Dump all command-line parameters and submission result such as returnCode, returnOut, " "jediTaskID, etc to a json file",
    )
    optP.add_argument("--cvmfs", action="store_const", const=True, dest="cvmfs", default=False, help="Bind /cvmfs to the container, bool, default False")
    optP.add_argument(
        "--noX509", action="store_const", const=True, dest="noX509", default=False, help="Unset X509 environment in the container, bool, default False"
    )
    optP.add_argument(
        "--datadir", action="store", dest="datadir", default="", help="Binds the job directory to datadir for I/O operations, string, default /ctrdata"
    )
    optP.add_argument("--workdir", action="store", dest="workdir", default="", help="chdir to workdir in the container, string, default /ctrdata")
    optP.add_argument(
        "--debug", action="store_const", const=True, dest="debug", default=False, help="Enable more verbose output from runcontainer, bool, default False"
    )
    optP.add_argument("--containerImage", action="store", dest="containerImage", default=None, help="Name of a container image")
    optP.add_argument(
        "--useCentralRegistry",
        action="store_const",
        const=True,
    ...
    optP.add_argument("--noSubmit", action="store_const", const=True, dest="noSubmit", default=None, help=argparse.SUPPRESS)
    optP.add_argument(
        "--outDS",
        action="store",
        dest="outDS",
        default=None,
        help="Base name of the output dataset container. Actual output dataset name is defined " "for each output file type",
    )
    optP.add_argument(
        "--outputs",
        action="store",
    ...
    optP.add_argument("--intrSrv", action="store_const", const=True, dest="intrSrv", default=None, help=argparse.SUPPRESS)
    optP.add_argument("--exec", action="store", dest="exec", default=None, help='execution string. e.g., --exec "./myscript arg1 arg2"')
    optP.add_argument("-v", "--verbose", action="store_const", const=True, dest="verbose", default=None, help="Verbose")
    optP.add_argument(
        "--priority",
        action="store",
        dest="priority",
    ...
    optP.add_argument(
        ...
        dest="useSandbox",
        default=False,
        help="To send files in the run directory to remote sites which are not sent out by default " "when --containerImage is used",
    )
    optP.add_argument("-3", action="store_true", dest="python3", default=False, help="Use python3")
