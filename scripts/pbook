...
For more info, do help(show) for example."""
    ...
    # retry
    def retry(JobID,upperJobID=None,newSite=False,newOpts={},noSubmit=False,ignoreDuplication=False):
        """Retry failed/cancelled subJobs in JobIDor JobsetID. Jobs between JobID and UpperJobID will be retried if upperJobID is given. Set newSite=True when you want to retry at new site, which essentially runs pathena/prun again because new sub jobs need to be created for the new site. This means that you need to have the same runtime env (such as Athena version, run dir, source files) as the previous submission.  newOpts is a map of options and new arguments which is passed to pathena/prun. If input files were used or are being used by other jobs for the same output dataset container, those file are skipped to avoid job duplication when retrying failed subjobs. If you need to disable duplication check (e.g., you are using the same EVNT file for multiple simulation subjobs, set ignoreDuplication=True
